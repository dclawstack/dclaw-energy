...
# Check events
kubectl get events -n dclaw-energy --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-energy deployment/dclaw-energy-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-energy-db-credentials -n dclaw-energy
   ```
