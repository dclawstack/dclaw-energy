...
### How do I update DClaw Energy?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp energy --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Energy without Kubernetes?

Yes, for local development:

```bash
cd dclaw-energy/frontend && npm run dev
cd dclaw-energy/backend && uvicorn dclaw_energy.main:app --reload
```
